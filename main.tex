...
%     \end{exampleblock} 
% \end{frame}
% (TODO) rework literature and potential application
...
\begin{frame}{Settings}
    \begin{itemize}
            \item \noPP: without preprocessing or any lifting
            \item \PP: with preprocessing a lifting
            \begin{itemize}
                \item \nPC and \nPCY: set upper bounds on $z^h$ and using lower bounds in \eqref{eq:nPC2-lift}
                \item \nPCE: problem initialized on reduced set $\mathcal{K}$ 
            \end{itemize}
            \item \sH: \PP with an additional starting heuristic
    \end{itemize}
\end{frame}
% \begin{frame}{Implementation}
%     \begin{itemize}
...
\begin{frame}{Setting comparison}
    \begin{figure}
        \begin{minipage}[c]{.48\linewidth}
            \subfloat[models optimality gap]{\includegraphics[width=\linewidth]{images/gap_models_sH.pdf}\label{fig:models_gap}{}}
        \end{minipage}
